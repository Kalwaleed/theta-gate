> **Update Sun 30 Aug 2026 (07:30 ET).** Read `docs/ANALYSIS-2026-08-30.md` first. The "no blockers" line below was wrong: as of `92b2f96` no order could ever be submitted (the CLI writes API errors to stderr; `alpaca._run` read stdout only) and the option chain was capped at 100 contracts. Both are fixed on `main` with 136 tests; the sign convention (item 5 below) is verified. The VRP gate vetoed every candidate on Friday's data; PK chose option C (`2bccbb4`): `vrp.realised_vol_lookback_days` 20 → 10 and `min_vrp_points` 2.0 → 1.0, so the realised-vol window matches the 6–9 DTE tenor. On Friday's marks SPY passes both expiries (+2.1 / +2.3 pts), QQQ only the 9-DTE (+1.3); Monday's open moves all four numbers. Chosen with Friday's data in view — say so in the write-up. PR #2 is closed; PRs #7–#12 are open.
...
Put-credit spreads on SPY/QQQ, 6–9 DTE, short delta 0.16–0.25, $5 wide, **1 contract Monday, 2 from Tuesday** (PK decision 30 Aug; the qty-2 change lands Monday after close with the partial-fill fix), entered only when 8–9 DTE ATM IV exceeds 10-day realised vol by ≥ 1.0 point (option C, 30 Aug), entries at 10:30 and 13:30 ET in 15-minute windows. Bearish proposals are NO_TRADE — V1 is put-only, never a call-side substitution. Last new entry Wed 2 Sep 10:45 ET; everything flattens Thu 3 Sep from 14:30 ET via a four-rung ladder; Fri 4 Sep is monitor-only ahead of the 11:00 ET submission. The LLM proposes an underlying and a direction and nothing else — strikes, sizing, and every gate are deterministic Python.
