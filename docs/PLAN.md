...
| `gate_cumulative_drawdown` | Equity ≤ **$98,000** (tightened from $96,000, canonical plan) → **no new entries.** It does NOT close anything: the gate sits in `risk.py`'s `_GATES`, the entry list, and returns a veto string — no code path liquidates on it, and open positions continue to exit on their own signals. Also fires on `trading_blocked` or three consecutive loop exceptions. |
